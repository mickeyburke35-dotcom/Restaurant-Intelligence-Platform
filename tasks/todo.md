...
## Competitor Observations MVP Plan

1. Keep the change limited to competitor service/API, competitor UI, this todo file, and README API documentation; do not modify authentication, Prisma schema, review import, review dashboard, AI generation, insight approval, or reports.
2. Add tenant-scoped competitor service functions to list competitors with user-entered observations and create observations against active competitors only, enforcing active agency and restaurant scope.
3. Add Zod-validated API routes for listing competitors and creating manual observations; no scraping, provider calls, AI generation, or fabricated competitor data.
4. Add a Competitors page with restaurant/location filters, empty state, safe error state, competitor list, and observation form/table showing date, channel/source, summary, sentiment, and evidence note.
...
- 2026-07-08: Implemented Competitor Observations MVP with focused competitor service/API/UI changes. Added tenant-scoped competitor listing and manual observation creation against existing active competitors, storing user-entered observation date, channel/source, summary, sentiment, evidence note, optional source URL, and signal type without scraping, external collection, AI generation, fabricated competitor data, Prisma schema changes, review import changes, review dashboard changes, insight approval changes, or report changes. Added `/competitors`, `GET /api/competitors`, and `POST /api/competitors/:competitorId/observations`; documented the new API behavior in README. `PROJECT_HANDOFF.md` was requested but was not present in the workspace; `AGENT_TEAM_HANDOFF.md` was read as the only available handoff file. Verified `npm run lint`, `npm run typecheck`, `npm run build`, and a local `HEAD /competitors` smoke check returning `200 OK`.
